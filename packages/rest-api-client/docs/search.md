...
| hits[].record         | Object | Present when `type` is `RECORD`, or when `type` is `ATTACHMENT` and `attachment.attachedTo` is `RECORD`. Contains `appId`, `appName`, `recordId`, `recordTitle`, `createdAt`, `creator`, `matchedFields` (`code` / `label`), and optionally `spaceId` / `spaceName`. |
...
| hits[].thread         | Object | Present when `type` is `THREAD`. Contains `spaceId`, `spaceName`, `threadId`, `threadName`, `createdAt`, and `creator`.                                                                                                                                              |
