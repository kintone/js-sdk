...
| httpsAgent                 |                                Object                                |                             | **This parameter is available only in Node.js environment.**<br />**Cannot be specified together with `dispatcher`.**<br />The custom HTTPS agent to be used when request. The agent should be compatible with [https.Agent](https://nodejs.org/api/https.html#class-httpsagent).<br />**Only its TLS options (`pfx`/`passphrase`/`cert`/`key`/`ca`/`rejectUnauthorized`) and `maxSockets` are honored** — any custom connection logic (e.g. an agent that tunnels through a SOCKS proxy) is not. Use `dispatcher` for that instead. |
| clientCertAuth             |                                Object                                |                             | **This parameter is available only in Node.js environment.**<br />**When `httpsAgent` or `dispatcher` parameter is given, this parameter is unavailable.**<br />If your Kintone environment uses [Client Certificate authentication](https://jp.cybozu.help/general/en/admin/list_security/list_access/secureaccess.html), please specify the certificate file and password.                                                                                                                                                         |
...
| proxy                      |                          Object or `false`                           |                             | **This parameter is available only in Node.js environment.**<br />**Cannot be specified together with `dispatcher`.**<br />If you use an HTTP or HTTPS proxy, please specify its configuration. `false` (the default) means no proxy is used.<br />For a SOCKS proxy, or any other connection strategy this option can't express, use `dispatcher` instead.                                                                                                                                                                          |
...
| dispatcher                 | [undici.Dispatcher](https://undici.nodejs.org/#/docs/api/Dispatcher) |                             | **This parameter is available only in Node.js environment.**<br />**Cannot be specified together with `proxy`, `httpsAgent`, or `clientCertAuth`.**<br />A custom [undici](https://undici.nodejs.org/) `Dispatcher`, used as-is to make requests. This is the escape hatch for connection strategies the other options can't express, such as a SOCKS proxy — build a `Dispatcher` whose `connect` tunnels through it and pass it here.                                                                                              |
