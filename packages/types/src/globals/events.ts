export type Events = {
  on<EventType extends keyof KintoneEventObjectTypes>(
    type: EventType,
    handler: (event: KintoneEventObjectTypes[EventType]) => void,
  ): void | Promise<void>;
  off<EventType extends keyof KintoneEventObjectTypes>(
    type?: keyof KintoneEventObjectTypes | keyof KintoneEventObjectTypes[],
    handler?: (event: KintoneEventObjectTypes[EventType]) => void,
  ): boolean | Promise<boolean>;
};

// イベントオブジェクトの型定義
type KintoneEventObjectTypes = {
  // レコード一覧画面
  // レコード一覧画面を表示した後のイベント
